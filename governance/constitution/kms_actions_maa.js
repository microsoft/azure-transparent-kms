      ...
      const add = (type, claims) => {
        ...
        if (ccf.kv[keyReleaseMapName].has(keyBuf)) {
          ...
          try {
            ...
            console.log(
              `[INFO] [scope=set_key_release_policy->add] KRP add ${type}=>Error parsing ${items} from key release policy during add`,
              e,
            );
            throw new Error(
        ...
        console.log(
          `[INFO] [scope=set_key_release_policy->add] KRP add ${type}=>Add claims to key release policy for ${type}: ${jsonItems}`,
        );
      ...
      const addOperator = (type, claims) => {
        ...
        if (ccf.kv[keyReleaseMapName].has(keyBuf)) {
          ...
          try {
            ...
            console.log(
              `[INFO] [scope=set_key_release_policy->add] KRP addOperator ${type}=>Error parsing ${items} from key release policy`,
              e,
            );
            throw new Error(
              `[ERROR] [scope=set_key_release_policy->add] Error parsing ${items} from key release policy during addOperator`,
              e,
            );
          }
        } else {
        ...
        Object.keys(claims).forEach((key) => {
          ...
          // Make sure item is always an array
          if (Array.isArray(item)) {
            throw new Error(`[ERROR] [scope=set_key_release_policy->add] The operator claim ${key} cannot be an array`);
          }
      ...
      const remove = (type, claims) => {
        ...
        if (ccf.kv[keyReleaseMapName].has(keyBuf)) {
          ...
          try {
            ...
            console.log(
              `[INFO] [scope=set_key_release_policy->remove] KRP remove ${type}=>Error parsing ${items} from key release policy`,
              e,
            );
            throw new Error(
              `[INFO] [scope=set_key_release_policy->remove] Error parsing ${items} from key release policy during remove`,
              e,
            );
          }
        } else {
          console.log(
            `[INFO] [scope=set_key_release_policy->remove] KRP remove ${type}=>key: ${type} does not exists in the key release policy`,
          );
          throw new Error(
        ...
        Object.keys(claims).forEach((key) => {
          ...
          if (items[key] !== undefined) {
            ...
            console.log(
              `[INFO] [scope=set_key_release_policy->remove] KRP remove ${type}=>Claim ${key} not found in the key release policy`,
            );
            throw new Error(
      ...
      const removeOperator = (type, claims) => {
        ...
        if (ccf.kv[keyReleaseMapName].has(keyBuf)) {
          ...
          try {
            ...
            console.log(
              `[INFO] [scope=set_key_release_policy->remove] KRP removeOperator ${type}=>Error parsing ${items} from key release policy`,
              e,
            );
            throw new Error(
              `[ERROR] [scope=set_key_release_policy->remove] Error parsing ${items} from key release policy during removeOperator`,
              e,
            );
          }
        } else {
        ...
        Object.keys(claims).forEach((key) => {
          ...
          // Make sure item is always an array
          if (Array.isArray(item)) {
            throw new Error(`[ERROR] [scope=set_key_release_policy->remove] The operator claim ${key} cannot be an array`);
          }

          if (items[key] !== undefined) {
            console.log(`[INFO] [scope=set_key_release_policy->remove] KRP remove ${type}=>Removing ${item} from ${key}`);
            delete items[key];
          } else {
            console.log(
              `[INFO] [scope=set_key_release_policy->remove] KRP remove ${type}=>Claim ${key} not found in the key release policy`,
